document.addEventListener('DOMContentLoaded', function() {
    ...
    // Helper function to parse set input (handles commas, newlines, and whitespace)
    function parseSetInput(input) {
        if (!input || typeof input !== 'string') return [];
        // Split by both commas and newlines, then trim and filter empty strings
        return input
            .split(/[,\n\r]+/)
            .map(x => x.trim())
            .filter(x => x.length > 0);
    ...
    if (vennBtn) {
        vennBtn.onclick = async function() {
            ...
            try {
                const sets = {};
                const a = parseSetInput(document.getElementById('setA').value);
                const b = parseSetInput(document.getElementById('setB').value);
                const c = parseSetInput(document.getElementById('setC').value);
