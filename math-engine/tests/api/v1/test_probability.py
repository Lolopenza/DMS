...
def test_union_probability(client):
    r = client.post(URL, json={
        'operation': 'union',
        'pA': 0.6,
        'pB': 0.5,
    ...
    assert r.status_code == 200
    assert abs(r.json()['result'] - 0.8) < 1e-9


def test_total_probability(client):
    r = client.post(URL, json={
    ...
    assert r.status_code == 200
    assert abs(r.json()['result'] - 0.38) < 1e-9


def test_distribution_summary_binomial(client):
    r = client.post(URL, json={
    ...
    assert r.status_code == 200
    data = r.json()['result']
    assert abs(data['mean'] - 5.0) < 1e-9
    assert abs(data['variance'] - 2.5) < 1e-9
    assert 'pmf_at_k' in data
