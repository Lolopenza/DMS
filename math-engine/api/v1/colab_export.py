...
def _build_lesson_markdown(user_id: int, window_days: int) -> str:
    ...
    system_prompt = (
        "You are an AI tutor writing concise notebook lesson text for a student learning "
        "discrete math and ML. Return markdown with exactly 3 sections: "
        "1) Interpret metrics, 2) Data pitfalls, 3) Discrete-math to ML bridge. "
        "Use practical examples and keep it under 220 words."
    )
    user_prompt = (
...
def build_colab_starter(req: ColabStarterRequest):
    ...
    nb.cells = [
        ...
        nbformat.v4.new_markdown_cell(
            ...
            "Logistic regression on your own data."
        ),
        nbformat.v4.new_code_cell(
            "model_df = df.copy()\n"
